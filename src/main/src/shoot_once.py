...
def talker(origin_frame):
  ...
  while not rospy.is_shutdown():
    temp_offset = raw_input('change offset?')
    if temp_offset:
      OFFSET = float(temp_offset)
    print(OFFSET)
    try:
      raw_input('Type to [open]  ')
      ...
      while not rospy.is_shutdown():
        try:
          raw_input("shaking")
          # trans_to_gun = tfBuffer.lookup_transform(origin_frame, "gun_frame", rospy.Time())
          ...
          # gun[0] = trans_to_gun.transform.translation.x
          # gun[1] = trans_to_gun.transform.translation.y
          # gun[2] = trans_to_gun.transform.translation.z
          ...
          gun[0] = (XY_OFFSET*trans_to_wrist.transform.translation.x+trans_to_tip.transform.translation.x)/(XY_OFFSET+1)
          gun[1] = (XY_OFFSET*trans_to_wrist.transform.translation.y+trans_to_tip.transform.translation.y)/(XY_OFFSET+1)
          gun[2] = (XY_OFFSET*trans_to_wrist.transform.translation.z+trans_to_tip.transform.translation.z)/(XY_OFFSET+1)
    ...
    #  to sprintf in C or MATLAB)
      pub_string = raw_input('Type to [close]: ')
    # Publish our string to the 'chatter_talk' topic
